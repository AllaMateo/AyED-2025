...
float Recinto::_suma(Sensor** v, int n, std::string unidad){
    ...
    if (n==this->nSensores) return 0.0;
        ...
        if (v[n]!=nullptr && [n]->getUnidad()==unidad) return _suma(v,n+1,unidad) + v[n]->getValor());
        else return _suma(v, n+1, unidad);        
        }
...
int Recinto::_maximo(Sensor** v, int n, std::string unidad, int mejorPos){
    ...
    if (n==this->nSensores) return 0;
        ...
        if (v[n]!=nullptr && [n]->getUnidad()==unidad) {
            if (v[mejorPos]->getValor() < v[n]->getValor()) 
                mejorPos = n;
            return _maximo(v,n+1,unidad,mejorPos) + v[n]->getValor();}
        else 
